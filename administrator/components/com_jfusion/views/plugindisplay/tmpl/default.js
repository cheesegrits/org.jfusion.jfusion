        ...
        new Request.JSON({
            ...
            onSuccess: function (JSONobject) {
                ...
                if (JSONobject.success ===  true && JSONobject.data && JSONobject.data.jname) {
                    var el = document.id(JSONobject.data.jname);
...
JFusion.submitForm = function (type) {
    new Request.JSON({
        ...
        format: 'json',
        onRequest: function () {
            document.id('spinner'+type).set('html', '<img border="0" alt="loading" src="components/com_jfusion/images/spinner.gif">');
        },
        onSuccess: function(JSONobject) {
            document.id('spinner'+type).set('html', '');
            JFusion.onSuccess(JSONobject);
    ...
    }).post(document.id('install'+type).toQueryString());
};

JFusion.downloadPlugin = function () {
    window.location = document.id('server_install_url').getSelected().get('value');
};

JFusion.updateList = function (JSONobject) {
    if (JSONobject.success ===  true && JSONobject.data && JSONobject.data.pluginlist) {
        var list = document.id('sort_table');
        list.empty();
...
JFusion.initSortables = function () {
    ...
    new Sortables('sort_table', {
        ...
        onComplete: function () {
            ...
            sortorder = '';
            document.iddocument.id('#sort_table tr').each(function (tr) {
...
window.addEvent('domready',function() {
    var server = document.id('installSERVER');
    if (server) {
        server.addEvent('submit', function (e) {
            e.stop();
            JFusion.submitForm('SERVER');
        });
    }

    document.id('installDIR').addEvent('submit', function(e) {
        e.stop();
        JFusion.submitForm('DIR');
    });

    document.id('installURL').addEvent('submit', function(e) {
        e.stop();
        JFusion.submitForm('URL');
    });

    var installZIP = document.id('installZIP');
    installZIP.addEvent('submit', function(e) {
        var upload;
        e.stop();
        document.id('spinnerZIP').set('html', '<img border="0" alt="loading" src="components/com_jfusion/images/spinner.gif">');
        //noinspection JSHint
        upload = new File.Upload({
            ...
            onComplete: function (result) {
                document.id('spinnerZIP').set('html', '');
